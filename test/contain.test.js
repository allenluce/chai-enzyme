...
describe('#contain', () => {
  ...
  describe('(nodes)', () => {
    it('passes when the actual matches the expected', (wrapper) => {
      expect(wrapper).to.contain([<User index={2} key='2' />, <User index={3} key='3' />])
    }, { render: false })

    it('passes negated when the actual does not match the expected', (wrapper) => {
      expect(wrapper).to.not.contain([<User index={3} key='2' />, <User index={4} key='4' />])
    }, { render: false })

    it('fails when the actual does not match the expected', (wrapper) => {
      expect(() => {
        expect(wrapper).to.contain([<User index={3} key='3' />, <User index={4} key='4' />])
      }).to.throw('to contain [\n  <User index={3} />,\n  <User index={4} />\n]')

      expect(() => {
        expect(wrapper).to.not.contain([<User index={2} key='2' />, <User index={3} key='3' />])
      }).to.throw('not to contain [\n  <User index={2} />,\n  <User index={3} />\n]')
    }, { render: false })

    it('fails when the actual is undefined', () => {
      expect(() => {
        expect(undefined).to.contain([<User index={2} key='2' />, <User index={3} key='3' />])
      }).to.throw()
    })
